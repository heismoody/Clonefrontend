...
export async function getServerSideProps(context) {
    ...
    const mediatype = String(context.query.mediatype);
    if (mediatype == "movie") {
                                            ...
                                            <h4 className="detailhead sm:text-xs">{
                                                    seasons.map((season, index) => {
                                                        if (index <= number_of_seasons) {
                                                            return <span className="border border-slate-300 rounded font-bold text-our-green mx-2 text-sm ">Season { season.season_number }</span>
                                                        }
                                                    })
                                                }</h4>
                                        }   

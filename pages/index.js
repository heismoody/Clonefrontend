          ...
          <div className="w-[1100px] flex justify-between px-5 sm:thumbmob">
            {results.map((result, index) => {
              ...
              const releasedate = moviedate.substring(0, 4);
                ...
                return <MovieThumb title={result.title} rate={result.vote_average} genres={genrenames} thumb={result.poster_path} year={releasedate} movieid={result.id} vote={result.vote_count} description={result.overview} mediatype={media_type} />
            }
          })}
          </div>
        </div>
